import type { Project, ProjectResult, UserProfile } from './types';

const art = (seed: string, hue: string) => {
  const palettes: Record<string, [string, string, string]> = {
    teal: ['#0e7780', '#142f3c', '#e66cff'],
    coral: ['#b95662', '#2a1f36', '#6ee6ff'],
    violet: ['#6d63b9', '#18263c', '#e9b84c'],
  };
  const [a, b, c] = palettes[hue] || palettes.teal;
  const n = seed.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0);
  const rotate = n % 28;
  return `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 580"><rect width="900" height="580" fill="${b}"/><circle cx="${160 + n % 170}" cy="${130 + n % 100}" r="190" fill="${a}" opacity=".95"/><circle cx="${650 - n % 130}" cy="${390 - n % 80}" r="220" fill="${c}" opacity=".9"/><path d="M0 470 C180 ${350 + rotate} 310 550 500 420 S760 320 900 430 V580 H0Z" fill="${a}" opacity=".8"/><path d="M40 90 L420 60 L500 150 L120 190Z" fill="${c}" opacity=".7"/><circle cx="${450 + rotate}" cy="280" r="64" fill="${b}" stroke="${a}" stroke-width="18"/><path d="M410 280 l28 28 58-70" fill="none" stroke="${a}" stroke-width="15" stroke-linecap="round" stroke-linejoin="round"/></svg>`)}`;
};

export const projects: Project[] = [
  ['p1','01','Tide Lines','A tiny coastal sensor that turns shoreline data into a living soundscape.','Our team built a low-cost sensor buoy that measures salinity, temperature and tide movement. The installation translates each reading into a changing musical composition, making an invisible ecosystem audible to visitors.','Earth & Environment','The Blue Current','1461151753365-8d4d7d15a7be','Water, science','Visual data'],
  ['p2','02','Borrowed Light','A solar reading nook designed for the school’s forgotten corners.','A modular reading seat that stores daylight during the afternoon and releases a soft glow after sunset. We combined recycled timber, flexible solar film and a simple battery circuit.','Design & Technology','Studio 4B','1500534623283-312aade485b7','Recycled materials','Solar circuit'],
  ['p3','03','The Memory Garden','A tactile garden that preserves the stories of people who shaped our school.','Visitors explore plantings chosen for their cultural meanings while listening to recorded memories from alumni, caretakers and neighbours. The garden grows as the community contributes.','Arts & Culture','Common Ground','1497250681960-ef046c4e94d2','Oral history','Community archive'],
  ['p4','04','Air We Share','Mapping invisible air currents through the school day.','A network of handmade paper pinwheels and open-source monitors reveals how air moves from classroom to classroom. The project pairs rigorous observation with a very human question: who gets the freshest air?','Science & Research','North Wing Lab','1535378917042-10a22c3a2f9c','Air quality','Open data'],
  ['p5','05','Second Skin','A collection of garments grown from food waste.','We experimented with bacterial cellulose made from fruit scraps, developing translucent sheets that can be stitched, dyed and composted at the end of their life.','Design & Technology','Mend / Make','1529139574466-a303027c1d8b','Bio-materials','Circular design'],
  ['p6','06','Small Acts, Big Map','A neighbourhood map built from everyday acts of care.','Every mark on this hand-drawn map represents a small act of care: a repaired bike, a shared meal, a borrowed book. It asks us to notice the infrastructure of kindness around us.','Social Inquiry','The Listening Post','1529156069898-49953e39b3ac','Field notes','Participatory map'],
  ['p7','07','Night Sky Radio','An interactive radio broadcast from the edge of the visible universe.','Tune across a room-sized radio to hear the mathematics of stars become rhythm and texture. The project makes astronomical scale intimate through sound, light and a lot of careful coding.','Science & Research','Signal / Noise','1444703686981-a3abbc4e11d8','Radio astronomy','Generative sound'],
  ['p8','08','Kitchen Table Atlas','Recipes and migration stories from our school community.','A printed atlas maps family recipes to the journeys that brought them here. Each page begins with a dish and opens into a story about memory, language and the places we carry.','Arts & Culture','Many Tables','1556911220-bff31c812dba','Food stories','Print atlas'],
  ...
  ['p10','10','Pocket Pollinators','A pocket-sized habitat kit for balconies and window ledges.','A kit of seed cards, nesting tubes and illustrated instructions helps people make small habitats wherever they live. We tested the kits with neighbours across three apartment blocks.','Earth & Environment','Wild Window','1497250681960-ef046c4e94d2','Urban ecology','Take-home kit'],
  ['p11','11','Language Weather','A forecast made from the words our school uses every day.','We collected anonymous snippets of school language and visualised their mood over a month. The result is a changing weather map that reflects how a community speaks to itself.','Social Inquiry','Word Watch','1499750310107-5fef28a66643','Text analysis','Community research'],
  ['p12','12','Repair Radio','A broadcast studio for fixing things and telling their stories.','Every repaired object has a history. Our pop-up radio station records those histories while teaching basic repair skills, creating a practical archive of how things stay useful.','Design & Technology','Fixers Collective','1518770660439-4636190af475','Repair culture','Live broadcast'],
].map(([id, number, title, shortDescription, fullDescription, category, teamName, seed, f1, f2]) => ({
  id, projectNumber: number, title, shortDescription, fullDescription, category, teamName,
  imageUrl: art(seed, 'teal'), isActive: true, isArchived: false, features: [f1, f2],
}));
