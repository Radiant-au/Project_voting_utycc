...
const emptyForm = { number: '', hiddenCode: '', title: '', short: '', full: '', team: '', category: projectCategories[0] as ProjectCategory, image: '' };
...
export function ProjectForm({ id }: { id?: string }) {
  ...
  useEffect(() => {
    if (!id) return;
    void (async () => {
      const { data, error: loadError } = await supabase.from('projects').select('project_number,hidden_project_code,title,short_description,full_description,team_name,category,image_url').eq('id', id).maybeSingle();
      setLoading(false);
      ...
      setForm({ number: data.project_number, hiddenCode: data.hidden_project_code, title: data.title, short: data.short_description, full: data.full_description, team: data.team_name, category: isProjectCategory(data.category) ? data.category : projectCategories[0], image: data.image_url || projectFallbackImage(id) });
    })();
  }, [id]);

  const submit = async (publish: boolean) => {
    const required = ['number', 'hiddenCode', 'title', 'short', 'full', 'team'];
    ...
    const project = { project_number: form.number.trim(), hidden_project_code: form.hiddenCode.trim().toUpperCase(), title: form.title.trim(), short_description: form.short.trim(), full_description: form.full.trim(), team_name: form.team.trim(), category: form.category, image_url: form.image || projectFallbackImage(form.number), is_active: publish };
    const result = id ? await supabase.from('projects').update(project).eq('id', id) : await supabase.from('projects').insert({ ...project, id: `p${Date.now()}`, features: [] });
    setSaving(false);
