...
type LiveRow = { rank: number; hidden_project_code: string; category: string; image_url: string; team_name: string; title: string; total_points: number };
...
export function AdminLiveDisplay() {
  ...
  const load = useCallback(async () => {
    const [{ data, error: loadError }, { data: settings, error: settingsError }] = await Promise.all([
      supabase.rpc('admin_live_top_projects'),
      supabase.from('voting_settings').select('results_revealed').eq('id', true).maybeSingle(),
    ]);
    if (loadError || settingsError || !settings) return setError('Live results are unavailable. Apply the latest database migration, then refresh.');
    setRows(data ?? []); setError('');
    setRevealed(settings.results_revealed);
  }, []);

  useEffect(() => {
    void load().finally(() => setLoading(false));
    const channel = supabase.channel('admin-live-results')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'votes' }, () => void load())
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'voting_settings' }, () => void load())
      .subscribe((status) => {
