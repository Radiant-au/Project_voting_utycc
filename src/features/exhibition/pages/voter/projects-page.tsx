...
export function ProjectsPage() {
  ...
  useEffect(() => {
    Promise.all([voterApi.session(), voterApi.projects()])
      .then(([sessionResponse, projectResponse]) => { setSession(sessionResponse.session); setItems(projectResponse.projects); })
  ...
  const exit = () => { void voterApi.logout().finally(() => router.replace('/')); };
  ...
  return <main className="utycc-page projects-page pb-32">
    ...
    {selectedProject && !voted && <GlassVoteBar project={selectedProject} onCancel={() => setSelected('')} onVote={() => router.push(`/projects/${selectedProject.id}?confirm=true`)} />}
