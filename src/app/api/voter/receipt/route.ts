...
export async function GET() {
  ...
  try {
    const { data, error } = await getVoterSupabase().from('votes').select('id,project_id,category,points,created_at').eq('id', session.voteId).maybeSingle();
    if (error || !data) return json({ error: 'not_found' }, 404);
    const project = await getProject(data.project_id);
    if (!project) return json({ error: 'not_found' }, 404);
    return json({ receipt: { voteId: data.id, category: data.category, points: data.points, createdAt: data.created_at, project } });
