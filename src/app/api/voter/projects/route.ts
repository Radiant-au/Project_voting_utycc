...
export async function GET() {
  ...
  try {
    const [projects, { data, error }] = await Promise.all([
      getProjects(),
      getVoterSupabase().from('voting_settings').select('is_open').eq('id', true).maybeSingle(),
    ]);
    if (error || !data) return json({ error: 'service_unavailable' }, 503);
    return json({ session: { category: session.category, hasVoted: session.hasVoted }, status: { isOpen: data.is_open }, projects });
  }
