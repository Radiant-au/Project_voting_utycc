...
const slots = [
  { qr: { x: 72.5, y: 312 }, code: { x: 118.38, y: 265, width: 136.74 } },
  { qr: { x: 359.4, y: 312 }, code: { x: 400.8, y: 265, width: 136.74 } },
  { qr: { x: 638.9, y: 312 }, code: { x: 683.24, y: 265, width: 136.74 } },
] as const;
...
export async function generateVisitorPassPdf(
  ...
  if (!templatePdf.getPageCount())
    throw new Error("The visitor PDF template has no pages.");
  ...
  for (let start = 0; start < codes.length; start += PASSES_PER_PAGE) {
    ...
    for (const [slotIndex, code] of codes
      .slice(start, start + PASSES_PER_PAGE)
      .entries()) {
      ...
      const qr = await output.embedPng(
